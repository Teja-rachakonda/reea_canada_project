...
/**
 * Call OpenAI for a plain-text chat reply, given a full message history.
 * Used by conversational modules (HouzGPT) where the answer is prose, not JSON.
 * @param {Array<{role:string, content:string}>} messages
 */
async function callOpenAIChat(messages, { maxTokens = 700, temperature = 0.6 } = {}) {
  if (!isConfigured()) {
    const err = new Error('OPENAI_API_KEY is missing or invalid in backend/.env (it should start with "sk-")')
    err.status = 500
    throw err
...
module.exports = { callOpenAIJson, callOpenAIChat, isConfigured, MODEL }
